def ReadPWMPARAM(pwm):
    ...
    with open(path) as f:
        ...
        pwm.set_pwm(1, 0, PWM_PARAM[1][2])
        pwm.set_pwm(3, 0, PWM_PARAM[0][2])
        print(PWM_PARAM)
    return PWM_PARAM
...
def Accel(PWM_PARAM,pwm,time,Duty):
    ...
    if Duty >= 0:
        ...
        pwm.set_pwm(1, 0, throttle_pwm)
    elif Duty == 0:
        pwm.set_pwm(1, 0, THROTTLE_STOPPED_PWM)
        time.sleep(0.01)
    else:
        #Need to Reverse -> Stop -> Reverse
        pwm.set_pwm(1, 0, THROTTLE_REVERSE_PWM)
        time.sleep(0.01)
        pwm.set_pwm(1, 0, THROTTLE_STOPPED_PWM)
        time.sleep(0.01)
